// go-parity runner: builtin corpus (TestBuiltin, against builtin test env).
// Go is the source of truth (parity/fixtures/builtin_mock.json).
...
import { setNowProviderForParity } from "../../../src/builtin/builtin.js";

const here = dirname(fileURLToPath(import.meta.url));
const fixture = join(here, "..", "..", "..", "parity", "fixtures", "builtin_mock.json");
...
// Builtin test env (JS analog of builtin_test.go env). int -> bigint, float -> number.
function builtinEnv(): Record<string, any> {
  return {
    ArrayOfString: ["foo", "bar", "baz"],
    ArrayOfInt: [1n, 2n, 3n],
    ArrayOfInt32: [1n, 2n, 3n],
    ...
    MixedNestedArray: [1n, [2n, 3n], [4.0, 5.0]],
    NestedInt32Array: [[1n, 2n], [3n, 4n]],
    // Go: &[]any{42} (pointer to slice); TS adapter: deref'd value [42n].
    PtrArrayWithNil: [42n],
  };
}
...
for (const row of rows) {
  if (row.bucket === "NOT_APPLICABLE") {
    test(`[builtin][N/A] ${row.expr} — ${row.reason}`, { skip: true }, () => {});
    continue;
  }
  test(`[builtin][${row.bucket}] ${row.expr}`, () => {
    ...
    if (row.nowMs !== undefined) {
      setNowProviderForParity(() => row.nowMs!);
    }
    try {
